import logging
from uuid import UUID
from sqlalchemy.orm import Session, joinedload
from src.db.models import Exam, Question, ExamSession, ExamStatus
from src.utils.exceptions import NotFoundError, ServiceError

class ExamService:
    def __init__(self, db_session: Session):
        self.db = db_session
        self.logger = logging.getLogger("Exam Service")

    def create_exam(self, title: str, course_id: UUID, semester_id: UUID, exam_type: str, duration: int = 120):
        try:
            course = (
                self.db.query(Course)
                .filter(Course.id == course_id)
                .one_or_none()
            )
            if not course:
                raise ServiceError("Course not found")

            course_code = course.code

            for _ in range(5):  # retry on collision
                exam_code = generate_exam_code(exam_type, course_code)

                exam = Exam(
                    title=title,
                    course_id=course_id,
                    semester_id=semester_id,
                    duration_minutes=duration_minutes,
                    exam_code=exam_code,
                )

                self.db.add(exam)
                self.db.commit()
                self.db.refresh(exam)

                return exam

        except IntegrityError:
            self.db.rollback()
            raise ServiceError("Failed to generate unique exam code")

        except Exception as e:
            self.db.rollback()
            self.logger.error(f"Failed to create exam: {e}")
            raise ServiceError("Failed to create exam")

    def get_exam_by_id(self, id: UUID):
        try:
            # Use joinedload to fetch related objects in one query
            exam = self.db.query(Exam).options(
                joinedload(Exam.course),
                joinedload(Exam.author)
            ).filter(Exam.id == id).first()

            if not exam:
                raise NotFoundError(f"Exam {id} not found")
            return exam
        except Exception as e:
            self.logger.error(f"Failed to fetch exam with id: {id} : {e}")
            raise ServiceError(f"Failed to fetch exam with id: {id} : {e}")

    def get_exams_by_course(self, course_id: UUID, semester_id: UUID):
        try:
            exams = self.db.query(Exam).options(
                joinedload(Exam.course),
                joinedload(Exam.author)
            ).filter(Exam.course_id == course_id, Exam.semester_id == semester_id).all()

            if not exams:
                raise NotFoundError(f"Exams for course {course_id} not found")
            return exams
        except Exception as e:
            self.logger.error(f"Failed to fetch exams for course: {course_id} : {e}")
            raise ServiceError(f"Failed to fetch exams for course: {course_id} : {e}")

    def get_exams_by_author(self, instructor_id: UUID):
        try:
            exams = self.db.query(Exam).options(
                joinedload(Exam.course),
                joinedload(Exam.author)
            ).filter(Exam.author_id == instructor_id).all()

            if not exams:
                raise NotFoundError(f"No exams by instructor {instructor_id}")
            return exams
        except Exception as e:
            self.logger.error(f"Failed to fetch exams for instructor: {instructor_id} : {e}")
            raise ServiceError(f"Failed to fetch exams for instructor: {instructor_id} : {e}")

    def add_question_to_exam(self, question_id, exam_id):
        try:
            question = self.db.query(Question).filter(Question.id == question_id).first()
            if not question:
                raise NotFoundError(f"Question {question_id} not found")

            question.exam_id = exam_id
            self.db.add(question)
            self.db.commit()
            self.db.refresh(question)
            return question
        except NotFoundError:
            raise
        except Exception as e:
            self.logger.error(f"Failed to add question {question_id} to exam: {e}")
            self.db.rollback()
            raise ServiceError(f"Failed to add question {question_id} to exam: {e}")

    def delete_question_from_exam(self, question_id, exam_id):
        try:
            question = (
                self.db.query(Question)
                .filter(Question.id == question_id, Question.exam_id == exam_id)
                .first()
            )
            if not question:
                raise NotFoundError(f"Question {question_id} not found")

            question.exam_id = None
            self.db.add(question)
            self.db.commit()
            self.db.refresh(question)
            return True
        except Exception as e:
            self.logger.error(f"Failed to add question to exam: {e}")
            self.db.rollback()
            raise ServiceError(f"Failed to add question to exam: {e}")


    def list_exams(self, title: str | None = None, limit: int = 50, offset: int = 0):
        try:
            query = self.db.query(Exam)
            if title:
                query = query.filter(Exam.title.ilike(f"%{title}%"))

            exams = query.limit(limit).offset(offset).all()
            return exams
        except Exception as e:
            self.logger.error(f"Failed to list exams: {e}")
            raise ServiceError(f"Failed to list exams: {e}")

    def list_exam_questions(self, exam_id: UUID):
        try:
            exam = self.db.query(Exam).filter(Exam.id == exam_id).first()
            if not exam:
                return None
            return exam.questions
        except Exception as e:
            self.logger.error(
                f"Failed to list questions from exam id: {exam_id} : {e}"
            )
            raise ServiceError(f"Failed to fetch exams : {e}")

    def update_exam(self, exam_id: UUID, title: str | None=None):
        try:
            updated_exam = self.db.query(Exam).filter(Exam.id == exam_id).first()

            if title:
                updated_exam.title = title

            self.db.add(updated_exam)
            self.db.commit()
            self.db.refresh(updated_exam)
            return updated_exam
        except Exception as e:
            self.logger.error(
                f"Failed to fetch exam with id: {exam_id} : {e}"
            )
            raise ServiceError(f"Failed to update exam details: {e}")

    def delete_exam(self, exam_id :str):
        try:
            exam = self.db.query(Exam).filter(Exam.id == exam_id).first()

            if exam:
                self.db.delete(exam)
                self.db.commit()
                return True
            return False
        except Exception as e:
            self.logger.error(
                f"Failed to fetch exam with id: {exam_id} : {e}"
            )
            raise ServiceError(f"Failed to update exam details: {e}")
